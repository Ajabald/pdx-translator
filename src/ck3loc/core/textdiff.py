...
def _markup_tokens(text: str) -> list[str]:
    ...
    tokens: list[str] = []
    for pattern in (RE_BRACKET, RE_DOLLAR, RE_ICON, RE_FMT_OPEN, RE_FMT_CLOSE):
...
def summarize_change(old: str, new: str, limit: int = 80) -> str:
    ...
    prefix = "косметическая правка" if kind == COSMETIC else "изменён текст"
    parts = []
    if removed:
        parts.append(f"убрано: {removed[:limit]}")
    if added:
        parts.append(f"добавлено: {added[:limit]}")
