...
COLUMNS = ("Файл", "Ключ", "Перевод", "В архиве с")
...
class ArchiveDialog(QDialog):
    def __init__(self, conn: sqlite3.Connection, parent=None):
        super().__init__(parent)
        self.conn = conn
        self.setWindowTitle("Архив старых переводов")
        self.setMinimumSize(900, 500)

        layout = QVBoxLayout(self)
        intro = QLabel(
            "Переводы ключей, которых больше нет в оригинале мода: удалённые строки "
        ...
        self.search.setPlaceholderText("по ключу, файлу или тексту перевода…")
        ...
        buttons = QHBoxLayout()
        copy_one = QPushButton("Копировать перевод")
        copy_one.clicked.connect(self._copy_selected)
        copy_all = QPushButton("Копировать всё (ключ + перевод)")
