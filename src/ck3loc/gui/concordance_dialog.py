...
class ConcordanceDialog(QDialog):
    def __init__(self, conn: sqlite3.Connection, fragment: str = "", parent=None):
        ...
        self.conn = conn
        self.setWindowTitle("Как переводили это раньше")
        ...
        row = QHBoxLayout()
        row.addWidget(QLabel("Фрагмент:"))
        self.search = QLineEdit(fragment)
        self.search.setPlaceholderText("слово или кусок фразы из оригинала…")
        self.search.setClearButtonEnabled(True)
        row.addWidget(self.search, 1)
        layout.addLayout(row)

        self.table = QTableWidget(0, 3)
        self.table.setHorizontalHeaderLabels(["Оригинал", "Перевод", "Источник"])
    ...
    def _reload(self) -> None:
        ...
        self.count_label.setText(
            "Ничего не найдено" if not records else
            f"Найдено: {len(records)} · двойной клик копирует перевод")

    def _copy_row(self, item) -> None:
        ru = self.table.item(item.row(), COL_RU)
        if ru is not None:
            QGuiApplication.clipboard().setText(ru.toolTip() or ru.text())
            self.count_label.setText("Перевод скопирован в буфер")
