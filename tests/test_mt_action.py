"""Перевод одной строки: Ctrl+M от нажатия до статуса.

Сеть здесь не открывается: в реестр провайдеров подставляется заглушка.
Проверяется сцепка целиком — действие, поток, запись, статус, откат, — потому
что рвётся она обычно не в ядре, а на стыках.
"""
...
class Shouty:
    """Заглушка сервиса: переводит в верхний регистр, в сеть не ходит."""
...
class Broken(Shouty):
    """Сервис, теряющий метки разметки."""
...
def window(tmp_path, make_tree, qtbot, monkeypatch):
    # одно хранилище на весь тест: новое на каждый вызов не помнило бы ничего
    fake = FakeSettings()
...
def select_key(win, key: str) -> int:
    """Поставить курсор на строку с этим ключом. Возвращает её unit_id."""
...
# --- отказ и сломанный ответ ---

def test_a_broken_answer_is_still_written_and_flagged(window, qtbot) -> None:
    """Строку надо показать человеку — значит, она должна существовать."""
