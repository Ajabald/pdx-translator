...
EN = 'l_english:\n a:0 "Hello"\n b:0 "World"\n'
# «Привет » — краевой пробел, которого в оригинале нет: строка переведена, но с
# замечанием. На счётчик прогресса это не влияет, зато чипу «!» есть что считать.
RU = 'l_russian:\n a:0 "Привет "\n'
...
def test_issue_chip_counts_rows_with_issues(window):
    assert window.chips.issues_chip.count == 1      # «Привет » с краевым пробелом
    assert "1" in window.chips.issues_chip.text()


def test_issue_chip_switches_the_filter_and_its_other_views(window):
    """Чип — витрина того же действия, что галка панели и пункт меню.

    Раньше так расходились чипы статусов: чип загорался, лишь когда кликнули по
    самому чипу, а фильтр, поставленный из меню, его не касался.
    """
