"""Окно настройки проверок: правки доходят до таблицы, а не остаются в окне.

Проверка вызывается из трёх мест — колонка «!», пересчёт одной строки и отчёт
F6. Раньше все три звали `qa.check_unit` без набора, то есть всегда со
встроенными значениями; настройка, не дошедшая хотя бы до одного из них,
разошлась бы в числах на одном и том же проекте.
"""
...
# «Привет » — краевой пробел, которого в оригинале нет: edge_space сработает
# при любых дефолтах, значит правило годится в подопытные.
EN = 'l_english:\n a:0 "Hello"\n b:0 "World"\n'
RU = 'l_russian:\n a:0 "Привет "\n b:0 "Мир"\n'
...
def window(qtbot, conn, project_file):
    ...
    qtbot.addWidget(win)
    # фоновый счётчик в тесте не нужен: он лезет в базу своим соединением
...
def test_unchecking_a_rule_disables_it_after_save(window, conn) -> None:
    ...
    # и это записано в файл проекта, а не только в память
...
def test_saved_overlay_is_a_delta_not_a_dump(window, conn) -> None:
    """Полный дамп заморозил бы набор на текущей версии приложения."""
    rule_item(window, "edge_space").setCheckState(0, Qt.Unchecked)
    window._save()
...
# --- проба на паре и примеры --------------------------------------------


def test_hit_counter_counts_the_project(project_file, conn) -> None:
    """Счётчик — половина смысла окна: по нему и решают, шумит ли правило.

    Гоняем `run()` прямо, без потока: проверяется запрос и подсчёт, а не
    механика QThread (она общая с `ScanWorker`).
    """
    ...
    assert scanned == 2                       # обе строки переведены
    assert counts.get("edge_space") == 1      # только «Привет »
...
def add_own_rule(monkeypatch, window, title="Ellipsis",
    ...
    """Завести своё правило так, как это делает пользователь.

    Диалог заведения модальный: не заглушив его, набор тестов встал бы на
    `exec()` намертво — ровно так уже вставал мастер первого запуска.
    """
...
def rules_under(item) -> set[str]:
    """Идентификаторы правил в поддереве — обход такой же рекурсивный."""
    found = set()
...
def test_tree_separates_built_in_rules_from_own(monkeypatch, window) -> None:
    """Главное, чего в окне не было видно: что чьё.

    Раньше категории шли плоским списком, и «Свои правила» выглядели такой же
    категорией, как «Разметка», — из окна нельзя было понять, у какого правила
    проверку не переписать.
    """
...
def test_language_group_names_the_language_of_the_project(window) -> None:
    """«Язык перевода» ничего не сообщает тому, кто и так переводит."""
    titles = []
...
def test_rules_of_another_language_are_shown_apart_with_a_reason(
    ...
    """Правило чужого языка молчит — без объяснения это выглядит поломкой."""
...
def test_a_built_in_rule_says_it_cannot_be_rewritten(window) -> None:
    ...
    # но настройка доступна — ради неё весь раздел и делался
...
def test_own_rule_reaches_the_check_after_save(monkeypatch, window, conn) -> None:
    ...
    # и записано целиком, а не дельтой: основания у своего правила нет
...
def test_rule_names_do_not_collide(monkeypatch, window) -> None:
    """Имя правила живёт в файлах — в оверлее и рядом с пометкой «не ошибка»."""
...
def test_a_rule_from_the_global_layer_cannot_be_deleted_here(
        monkeypatch, window) -> None:
    """Оно задано на все проекты — в проекте его можно только выключить."""
    tab = window.rules_tab
    tab.scope_combo.setCurrentIndex(tab.scope_combo.findData(GLOBAL))
    rule_id = add_own_rule(monkeypatch, window)
    window._save()
    ...
    # само правило осталось жить в глобальном слое
...
def test_own_rule_can_be_duplicated(monkeypatch, window) -> None:
    ...
    assert copy.params["chars"] == "…"          # копия работает так же
    assert tab._rules.get(rule_id) is not None  # оригинал на месте
...
def test_a_hand_set_rule_is_marked_in_the_tree(window) -> None:
    """Иначе «вернуть к набору» бьёт вслепую: не видно, что отличается."""
    tab = window.rules_tab
    item = rule_item(window, "edge_space")
    tab.tree.setCurrentItem(item)       # кнопка сброса относится к выбранному
...
def test_returning_built_in_rules_keeps_own_ones(monkeypatch, window) -> None:
    """Прежняя «Сбросить всё» удаляла свои правила заодно — молча."""
    yes(monkeypatch)
    rule_id = add_own_rule(monkeypatch, window)
    tab = window.rules_tab
    rule_item(window, "edge_space").setCheckState(0, Qt.Unchecked)
    ...
    tab._reset_base()
    assert tab._rules.get("edge_space").enabled          # базовое вернулось
    assert tab._rules.get(rule_id) is not None           # своё осталось
...
def test_deleting_own_rules_is_a_separate_action(monkeypatch, window) -> None:
    ...
    assert not tab._rules.get("edge_space").enabled      # настройка не тронута


def test_changing_the_preset_does_not_take_own_rules_with_it(
        monkeypatch, window) -> None:
    """Пресет — про строгость встроенных проверок, о своих он не говорит ничего."""
    rule_id = add_own_rule(monkeypatch, window)
    tab = window.rules_tab
    tab.preset_combo.setCurrentIndex(tab.preset_combo.findData("quiet"))
...
def test_import_asks_before_replacing(monkeypatch, window, tmp_path) -> None:
    """Приехавший набор заменяет слой целиком — молча такое делать нельзя."""
    from PySide6.QtWidgets import QMessageBox
...
def test_the_empty_tab_explains_what_it_is_for(window) -> None:
    """Пустой список вызывал вопрос вслух — значит, он на него не отвечал."""
...
def test_preset_from_the_menu_changes_the_issues_column(
    ...
    Ради неё всё и затевалось: раньше модель считала замечания встроенными
    значениями и настройку не видела вовсе.
    """
    monkeypatch.setattr(settings, "recent_projects", lambda: [])
    monkeypatch.setattr(settings, "last_project_path", lambda: None)
    monkeypatch.setattr(settings, "remember_project", lambda *a, **k: None)
...
def test_window_opens_without_a_project(qtbot) -> None:
    """Набор правил глобальный — запрещать его настройку до открытия мода
    было бы странно, а счётчик срабатываний просто молчит."""
